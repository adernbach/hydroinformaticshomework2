...
def SNOTELPlots(sitedict, gdf_in_bbox, WY, watershed, AOI, DOI,plot = True):
    ...
    # Build a code-to-name mapping for lookups
    code_to_name = dict(zip(gdf_in_bbox['code'], gdf_in_bbox['name']))

    axs = axs.ravel()
    for i, key in enumerate(sitedict.keys()):
        df = sitedict[key]

        # Try to find site name by code; if key is renamed, use key as fallback
        site_name = code_to_name.get(key, key)
        axs[i].set_title(f"SNOTEL Site: {site_name}")
        ...
        if f"{WY}_SWE_in" in df.columns:
            ...
            # Only calculate percentages if DOI values exist
            if doivalue is not None and doimed is not None:
